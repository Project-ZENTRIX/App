...
1. [ ] 实现商品查询接口，明确课包商品与会员商品的返回结构。
2. [ ] 实现订单创建、订单列表、订单详情与取消订单接口。
3. [ ] 实现继续支付与支付状态查询接口。
4. [ ] 实现会员订阅查询、创建、续费与自动续费管理接口。
5. [ ] 实现支付 webhook，完成订单与订阅状态同步。
...
1. [ ] 实现学习进度总览接口。
2. [ ] 实现课程、课时和任务维度的进度查询接口。
3. [ ] 实现进度事件上报与聚合接口。
4. [ ] 实现成就列表、用户成就、等级定义与等级进度查询接口。
5. [ ] 对齐 Web 控制台的统计卡片、成就墙和进度明细展示字段。
